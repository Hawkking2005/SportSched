...
class ReservationViewSet(viewsets.ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        try:
            ...
            # Check for overlapping reservations across all facilities
            ...
            if overlapping_reservations:
                return Response(
                    {"detail": "You already have a reservation at this time slot. You cannot book multiple facilities at the same time."},
                    status=status.HTTP_400_BAD_REQUEST
                )

            # Check for overlapping reservations that end during this time slot
            overlapping_end_reservations = Reservation.objects.filter(
                user=request.user,
                is_cancelled=False,
                time_slot__date=time_slot.date,
                time_slot__end_time__gt=time_slot.start_time,
                time_slot__start_time__lt=time_slot.start_time
            ).exists()

            if overlapping_end_reservations:
                return Response(
                    {"detail": "You have another reservation that overlaps with this time slot."},
                    status=status.HTTP_400_BAD_REQUEST
                )

            # Check for reservations that start during this time slot
            overlapping_start_reservations = Reservation.objects.filter(
                user=request.user,
                is_cancelled=False,
                time_slot__date=time_slot.date,
                time_slot__start_time__lt=time_slot.end_time,
                time_slot__start_time__gt=time_slot.start_time
            ).exists()

            if overlapping_start_reservations:
                return Response(
                    {"detail": "You have another reservation that overlaps with this time slot."},
                    status=status.HTTP_400_BAD_REQUEST
                )
            ...
            time_slot.save()  # This will trigger the court availability update
